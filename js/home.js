document.addEventListener('DOMContentLoaded', () => {
  const categoriesImgs = Array.from(document.getElementsByClassName('slides')[0].children);
    ...
    categoriesImgs.forEach(element => {
      element.addEventListener('click', () => {
        let index // #7f8c8d
        if (navLinks[0].classList.contains('active')) { index = 0 }
        else if (navLinks[1].classList.contains('active')) { index = 1 }
        else if (navLinks[2].classList.contains('active')) { index = 2 }
        else { index = 3 }
        localStorage.setItem('currentCategoryId', index)
